...
(function (global) {
    ...
    function mount(el, opts) {
        if (!el) return null;
        opts = opts || {};
        var want = el.getAttribute('data-land-mount');
        if (want && want !== 'CF-main') return null;

        // #captcha-root: один раз и навсегда (запрет remount / чужого бренда)
        var isLandRoot = el.id === 'captcha-root'
            || (el.classList && (el.classList.contains('cf-captcha-shot') || el.classList.contains('re-captcha-shot')));
        if (isLandRoot) {
            try {
                if (global.__LAND_WIDGET_LOCK && global.__LAND_WIDGET_LOCK !== 'CF-main') {
                    return null;
                }
            } catch (eLock) {}
            var already = el.getAttribute('data-land-mounted');
            if (already) {
                return el.getAttribute('data-ray-id') || already;
            }
        }
        ...
        if (isLandRoot) {
            try {
                global.__LAND_WIDGET_LOCK = 'CF-main';
            } catch (eL2) {}
        }
    ...
    function auto() {
        try {
            if (global.__LAND_WIDGET_LOCK) return;
        } catch (e0) {}
        var el = document.querySelector('[data-land-mount="CF-main"]');
